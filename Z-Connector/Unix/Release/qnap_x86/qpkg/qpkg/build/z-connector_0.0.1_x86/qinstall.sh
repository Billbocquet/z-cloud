...
############################################################################
#
# $Id: qinstall.sh 377 2012-06-14 09:34:20Z micke $
#
...
######################################################################
# The installed QPKG applications' init-scripts are not called in
# correct order, so to minimize the possible problems we install a
# script that before a reboot/shutdown makes sure the qpkg.conf has a
# valid format.
######################################################################
create_update_qpkg_conf_script(){
	[ -x $SYS_QPKG_INSTALL_PATH/update_qpkg_conf.sh ] && return

	$CMD_CAT > $SYS_QPKG_INSTALL_PATH/update_qpkg_conf.sh <<-EOF
	#!/bin/sh

	case "\$1" in
		start)
			# Hide the QPKG and add correct symlink last in sequence.
			$CMD_SED -i '/\[update_qpkg_conf\]/,\${/^Name = .*/d}' $SYS_QPKG_CONFIG_FILE
			$CMD_RM -f $SYS_SHUTDOWN_DIR/QK*update_qpkg_conf
			$CMD_LN -sf "$SYS_QPKG_INSTALL_PATH/update_qpkg_conf.sh" "$SYS_SHUTDOWN_DIR/QK999update_qpkg_conf"
			;;
		stop)
			DATA_DIR=$SYS_QPKG_INSTALL_PATH/.data
			$CMD_RM -fr \$DATA_DIR
			$CMD_MKDIR \$DATA_DIR
			$CMD_AWK 'BEGIN{RS="\[";FS="\n"}
			{
			...
			$CMD_RM \$DATA_DIR/update_qpkg_conf

			for f in \$DATA_DIR/*
			do
				$CMD_SED -i '\$d' \$f
				rc_num=\$($CMD_SED -n 's/RC_Number = \(.*\)/\1/p' \$f)
				if [ -z "\$rc_num" ]; then
					rc_num=\$($CMD_FIND $SYS_STARTUP_DIR -type l -name "QS*\${f##*/}" | $CMD_SED "s#.*QS\(.*\)\${f##*/}#\1#")
					[ -n "\$rc_num" ] && echo "RC_Number = \$rc_num" >> \$f
				fi
				echo "\${rc_num:-9999}:\${f##*/}" >> \$DATA_DIR/index
			done
			$CMD_SORT -n \$DATA_DIR/index > \$DATA_DIR/sorted

			for f in \$($CMD_SED 's/.*:\(.*\)/\1/' \$DATA_DIR/sorted)
			do
				$CMD_CAT \$DATA_DIR/\$f >> \$DATA_DIR/qpkg.conf
			done
			echo "[update_qpkg_conf]" >> \$DATA_DIR/qpkg.conf
			echo "Name = update_qpkg_conf" >> \$DATA_DIR/qpkg.conf
			echo "Shell = $SYS_QPKG_INSTALL_PATH/update_qpkg_conf.sh" >> \$DATA_DIR/qpkg.conf
			$CMD_CP -pf $SYS_QPKG_CONFIG_FILE ${SYS_QPKG_CONFIG_FILE}.bak
			$CMD_MV \$DATA_DIR/qpkg.conf $SYS_QPKG_CONFIG_FILE
			$CMD_RM -fr \$DATA_DIR
			;;
		restart)
			\$0 stop
	...
	EOF
	$CMD_CHMOD +x $SYS_QPKG_INSTALL_PATH/update_qpkg_conf.sh
	$CMD_RM -f $SYS_SHUTDOWN_DIR/QK*update_qpkg_conf
	$CMD_LN -sf "$SYS_QPKG_INSTALL_PATH/update_qpkg_conf.sh" "$SYS_SHUTDOWN_DIR/QK999update_qpkg_conf"
}
